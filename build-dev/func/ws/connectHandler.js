...
async function registerNotificationClientsConnection({ sessionUser, connectionId, }) {
...
async function registerChatClientsConnection({ sessionUser, connectionId }) {
    let chatGroupIds = [];
...
const runHandle = async (event) => {
    try {
        ...
        const token = event?.queryStringParameters?.[SocketAuthKeys.token];
        if (!token) {
        ...
        try {
            ...
            const connectionKind = event?.queryStringParameters?.[SocketAuthKeys.connection_kind];
            const applicationId = event?.queryStringParameters?.[SocketAuthKeys.applicationId];
            const platform = event?.queryStringParameters?.[SocketAuthKeys.platform];
            const appVersion = event?.queryStringParameters?.[SocketAuthKeys.app_version];
...
//# sourceMappingURL=connectHandler.js.map