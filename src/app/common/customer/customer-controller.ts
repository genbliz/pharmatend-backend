...
async function getById(req: Request, res: Response) {
  try {
    const { tenantId } = await BaseController.getSessionUserInfo({
      ...
      withAnyPermission: [
        /* == */
        BaseController.DefinedRequiredPermission.customer.view,
      ],
    });
    ...
    const result = await CustomerRepository.findSingle({ tenantId, dataId });
    ...
    if (BaseController.isNewData(data)) {
      await BaseController.checkValidateHasPermission({
        ...
        requiredAnyPermission: [
          /* == */
          BaseController.DefinedRequiredPermission.customer.add,
        ],
      });
      result = await CustomerRepository.save({ data, sessionUser });
      //
    } else {
      await BaseController.checkValidateHasPermission({
        req,
        requiredAnyPermission: [
          /* == */
          BaseController.DefinedRequiredPermission.customer.edit,
        ],
      });
      result = await CustomerRepository.update({ data, sessionUser });
    }
...
async function deleteData(req: Request, res: Response) {
  try {
    const sessionUser = await BaseController.getSessionUserInfo({
      ...
      withAnyPermission: [
        /* == */
        BaseController.DefinedRequiredPermission.customer.delete,
      ],
    });
    ...
    const result = await CustomerRepository.delete({ dataId, sessionUser });
...
async function query(req: Request, res: Response) {
  try {
    ...
    const builder = CustomerRepository.base_queryBuilder();
    ...
    if (isPaging === "true") {
      const result = await CustomerRepository.base_getWherePaging({
        tenantId,
        nextPageHash,
        ...
        fields: CustomerModel.getLiteFields(),
      });
    ...
    const result = await CustomerRepository.base_getWhere({
      tenantId,
      query: builder.buildQuery(),
      limit: builder.props.count,
      sortKeyParams: {
...
//
export const CustomerRouter = myRouter;
