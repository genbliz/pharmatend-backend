...
class UserRepositoryBase extends CoreTenantRepository<IUser> {
  ...
  async createRegisterUser({
    ...
    const plainUser = UtilService.convertObjectToJsonPlainObject(userData) as IUser;
  ...
  async updateMyInfo({ user, sessionUser }: { user: IUser; sessionUser: ISessionUser }): Promise<IUser> {
  ...
  private async preFormatBeforeSaveUpdateData(data01: IUser) {
    if (data01.phone) {
  ...
  async updateUserInfoByAdmin({ user, sessionUser }: { user: IUser; sessionUser: ISessionUser }): Promise<IUser> {
    const isUserAnAdministrator: boolean = await AdminService.isAdministratorUser({ sessionUser });
  ...
  async getUsersValueListByTenantId({ tenantId }: { tenantId: string }): Promise<ITextValue<string>[]> {
  ...
  getUsersByTenantId({ tenantId, fields }: { tenantId: string; fields?: (keyof IUser)[] }) {
  ...
  getByUserKind({ tenantId, userKindTypes }: { tenantId: string; userKindTypes: UserKindsEnum[] }) {
    return this.base_getWhere({
      tenantId,
      query: {
        userKind: {
  ...
  async checkPasswordByUserData({ user, password }: { user: IUser; password: string }): Promise<boolean> {
  ...
  async existsByEmail({ email, tenantId }: { email: string; tenantId: string }): Promise<boolean> {
  ...
  async existsByUserName({ userName, tenantId }: { userName: string; tenantId: string }): Promise<boolean> {
    const result = await this.base_getOneByTenantIdAndCondition({
      tenantId,
      query: { $or: [{ userName: userName }, { userName: userName.toLowerCase() }] },
      fields: ["id", "userName"],
  ...
  async existsByPhone({ phone, tenantId }: { phone: string; tenantId: string }): Promise<boolean> {
    const result = await this.base_getOneByTenantIdAndCondition({
      tenantId,
      query: { phone },
      fields: ["id", "phone"],
  ...
  async adminChangeAnyUserPassword({
    userId,
    newPassword,
    confirmPassword,
    sessionUser,
