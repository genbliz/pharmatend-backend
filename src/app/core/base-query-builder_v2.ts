...
import { IManyQueryTypeAll } from "@/core/base-query-type.js";
...
export class MyQueryBuilderV2<T> {
  private query: IMocodyQueryDefinition<IManyQueryTypeAll<T>>;
  ...
  constructor(query?: IMocodyQueryDefinition<IManyQueryTypeAll<T>>) {
  ...
  /** Add query object */
  addQuery({ query }: { query: IMocodyQueryDefinition<IManyQueryTypeAll<T>>; }) {
    if (query && typeof query === "object") {
      this.query = {
        ...this.query,
        ...query,
