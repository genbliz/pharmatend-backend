...
import { IMessageSocketBody, SocketBodyKindEnum } from "@/common/chat/chat-types.js";
import { IChatItem, IChatItemExtra } from "@/common/chat/chat-item/chat-item-types.js";
import { ConnectionHelperService } from "#func/ws/connection-helper-service.js";
import { IRequiredAwsRequestParameters } from "#func/ws/types.js";
import { ChatInstantRepository } from "@/common/chat/chat-instant/chat-instant-repository.js";
import { IChatInstant } from "@/common/chat/chat-instant/chat-instant-types.js";
...
async function sendSocketMessage({
  ...
  myRequestParams: IRequiredAwsRequestParameters;
  connectionIds: string[];
  msgTypeKind: SocketBodyKindEnum;
  data: string | Record<string, any> | Record<string, any>[];
  chatRequestId?: string;
}) {
  const { domainName, stage } = myRequestParams;

  const socketBody: IMessageSocketBody = {
    msgTypeKind,
    bodyData: data,
  };
...
async function sendInstantChatToRoom({
  myRequestParams,
  data,
  chatRequestId,
  currentConnection,
  ...
  data: IChatInstant;
  chatRequestId?: string;
  currentConnection: ISocketConnection;
}) {
  try {
    if (!data?.receiverId) {
...
export async function handleInstantChatSendFromClient({
  myRequestParams,
  data,
  chatRequestId,
  currentConnection,
...
async function handleChatItemSendFromClient({
  myRequestParams,
  data,
  chatRequestId,
  currentConnection,
  ...
  myRequestParams: IRequiredAwsRequestParameters;
  data: { chatGroupId?: string; message: string };
  chatRequestId?: string;
  currentConnection: ISocketConnection;
}) {
  try {
    ...
    try {
      const chatItem: IChatItemExtra = await ChatItemRepository.create({
        chatGroupId: data.chatGroupId,
        message: data.message,
        sessionUser: { ...currentConnection, organizationId: "" },
      });
...
async function handleChatSendActiveUsersIds({
  myRequestParams,
  chatRequestId,
  currentConnection,
}: {
  ...
  let currentActiveUsersIds: string[] = [];
...
async function notifyPlainMessageToOtherConnectedUsers({
  myRequestParams,
  chatRequestId,
  currentConnection,
  userIds,
...
async function handleSendNotificationToParticipantsOrGroup({
  myRequestParams,
  chatRequestId,
  currentConnection,
  chatGroupId,
  ...
  try {
    ...
    let participantIdsValues: string[] = [];
...
async function handleAnyMessage({
  msgTypeKind,
  myRequestParams,
  data,
  chatRequestId,
...
async function handleChatGroupCreated({
  myRequestParams,
  chatGroupId,
  chatRequestId,
  currentConnection,
  ...
  try {
    ...
    // const participantIds = [...chatGroup.participantIds];

    // const activeUsersInGroup = tenantConnectedActiveUsers.filter((con) => participantIds.includes(con.userId));
    ...
    const activeUserInGroupUpdated: ISocketConnection[] = [];
    ...
    for (const activeUserInGroup of tenantConnectedActiveUsers) {
      if (activeUserInGroup?.id) {
        ...
        // LoggingService.log({ mesage: "Updated connection store!!", activeUserInGroup });
...
type IInstantChatBodyData = {
  receiverId: string;
  message: string;
};

const runHandle: APIGatewayProxyWebsocketHandlerV2 = async (event) => {
  const currentSessionRequestParams: IRequiredAwsRequestParameters = {
  ...
  const socketBody = formatChatRequestData<IChatBodyData>(event.body);
  ...
  const isValidConnection = !!(
    currentConnection.dangerouslyExpireAt && new Date(currentConnection.dangerouslyExpireAt).getTime() > new Date().getTime()
  );
  ...
  if (socketBody.msgTypeKind === SocketBodyKindEnum.HEART_BEAT) {
    try {
      const socketBodyHeartBeat = formatChatRequestData<string>(event.body);
  ...
  if (socketBody.msgTypeKind === SocketBodyKindEnum.INSTANT_CHAT_SEND) {
    try {
      const socketBody01 = formatChatRequestData<IInstantChatBodyData>(event.body);
