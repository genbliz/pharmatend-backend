...
class ConnectionLambdaCacheHelperServiceBase extends LocalCacheService {
  ...
  async getConnection({ connectionId }: { connectionId: string }) {
    const currentConnection01 = await super.readJsonFile<ISocketConnection>({ fileId: connectionId });
  ...
  async setConnection({ connectionId, data }: { connectionId: string; data: ISocketConnection }) {
