...
async function starter() {
  ...
  const stackName = "tendpos-backend-v2-staging";

  const { parsedEnv, environmentalVariableKeys } = await loadEnvironmentalVariable("staging");
  ...
  console.log(JSON.stringify(domainMapOption || {}, null, 2));

  if (!domainMapOption) {
    throw new Error("apigateway domain (staging) not found. You MUST create it manually");
  }
  ...
  if (!webSocketDomainMapOption) {
    throw new Error("apigateway websocket (staging) domain not found. You MUST create it manually");
  }

  new AppDeploymentStack(app, stackName, {
    env: { region: parsedEnv.APP_AWS_REGION },
    stackName,
